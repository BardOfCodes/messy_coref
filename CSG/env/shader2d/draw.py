...
from .constants import PRIMITIVE_OCCUPANCY_MAX_THRES, PRIMITIVE_OCCUPANCY_MIN_THRES, BBOX_LIMIT
...
class ColoredDraw2D:
    ...
    def __init__(self, resolution=64, device="cpu"):
        ...
        self.device = device
        self.resolution_tensor = th.FloatTensor([self.resolution])
        self.zero_tensor = th.FloatTensor([0])
    ...
    def set_to_cuda(self, set_to_half=True):
        self.device = "cuda"
    ...
    def set_to_half(self):
        ...
        self.tensor_type = th.HalfTensor
        # self.tensor_type = th.FloatTensor
    ...
    def translate(self, param=[0,0], coords=None):
        ...
        if not isinstance(param, th.autograd.Variable):
            param = self.tensor_type(param).to(self.device)
    ...
    def rotate(self, param=[0], coords=None, units="degree", inverted=False):
        ...
        rotation_matrix = rotation_matrix.repeat(coords.shape[0], 1, 1)
...
#         vec2[:,:,:,0] -= r
#         vec2[:,:,:,1] -= h
        sdf = th.amax(vec2, 3) + th.norm(th.clip(vec2, min=0.0) + 1e-9, -1)
        return sdf
    ...
    def mirror_coords(self, param=[1, 1, 1], coords=None, mode="euclidian"):
        ...
        param = param/(th.norm(param) + 1e-9)
    ...
    def shape_rotate(self, param, input_shape, inverted=True, units='degree'):
        # Assume Shape to be a RES, RES, RES, 2 object
    ...
    def _shape_sample(self, grid, input_shape):
        ...
        input_shape_sampling = input_shape_sampling.float()
