from .parser_utils import  draw_commands, boolean_commands, transform_commands
from .parser_utils import mcsg_commands_to_lower_expr, fcsg_to_mcsg
from CSG.env.csg3d.constants import TRANSLATE_RANGE_MULT, SCALE_RANGE_MULT, ROTATE_RANGE_MULT
from CSG.env.csg3d.constants import TRANSLATE_MIN, TRANSLATE_MAX, ROTATE_MIN, ROTATE_MAX, SCALE_MIN, SCALE_MAX, ROTATE_MULTIPLIER, SCALE_ADDITION, CONVERSION_DELTA
import os
import torch as th
import numpy as np
...
class HCSG2DParser(MCSG2DParser):
...
class FCSG2DParser(MCSG2DParser):
    """ Does not contain any macros
    """
                ...
                if n_param > 0:
                    ...
                    for ind, command_symbol in enumerate(self.transform_sequence):
                        variable, param_th = self.create_normalized_tensors(
                            param[ind*2: (ind+1)*2], command_symbol)
    ...
    def sample_only_primitive(self, valid_draws=draw_commands):
        ...
        n_param = len(self.transform_sequence) * 2
        ...
        if "rotate" in self.transform_sequence:
            rand_var = np.random.beta(2, 3, size=1)
...
class PCSG2DParser(FCSG2DParser):
    """ Does not contain any macros
    """
    ...
    def load_language_specific_details(self):
        ...
        self.trivial_expression = ["sphere(0, 0, 0.5, 0.5)", "$"]

        self.transform_sequence = ["translate", "scale"]

        self.has_transform_commands = False
        self.fcsg_mode = False
        self.load_fixed_macros("box_edges.pcsg2d")



class RotationFCSG2DParser(FCSG2DParser):
    """ Does not contain any macros
    """
        ...
        for expr in expression_list:
            ...
            if command_symbol == "macro":
                ...
                if n_param > 0:
                    ...
                    command_dict['param'] = param[2*2: 3*2]
        ...
        for expr in expression:
            ...
            if command_symbol in boolean_commands: 
                ...
                for ind, transform in enumerate(self.transform_sequence[:2]):
                    sel_param = param[ind * 2: (ind+1) * 2]
                    cur_param_str = ", ".join([str(x) for x in sel_param])
                    command = "%s(%s)" % (transform, cur_param_str)
                    mcsg_expr.append(command)
                # Now add the rotation and change the draw
                transform = self.transform_sequence[2]
                sel_param = param[2 *2: 3 * 2]
                cur_param_str = ", ".join([str(x) for x in sel_param])
                command = "%s_%s(%s)" % (transform, command_symbol, cur_param_str)
                mcsg_expr.append(command)
