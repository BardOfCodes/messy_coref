...
from CSG.env.csg3d.generator_helper import get_permutation_index_tuple, get_valid_actions, valid_operation, is_valid_expr, execute
from CSG.env.csg3d.constants import VALIDATION_THRESHOLD as THRESHOLD, VALIDATION_THRESHOLD_DIFF as DIFF_THRESHOLD, TRANSFORM_SAMPLING_RATE, MIRROR_SAMPLING_RATE, HIER_TFORM_RATE
...
def sample_random_primitive(parser, compiler, valid_draws,
                            valid_transforms, lang_type="CSG2D", 
                            allow_mirror=False, allow_mirror_macro=False):
    
    count = 0
    while(True):
        if lang_type in ["MCSG2D", "HCSG2D"]:
            ...
            if lang_type == "MCSG2D":
                if np.random.sample() <= MIRROR_SAMPLING_RATE:
                    if allow_mirror:
                        ...
                        expr = transform_expr + expr
        elif lang_type in ["PCSG2D", "FCSG2D"]:
            expr = parser.sample_random_primitive(valid_draws)
        elif lang_type in ["HCSG2D", "MCSG2D"]:
            expr = parser.sample_random_primitive(valid_draws)
...
def generate_programs_func(proc_id, program_lengths, parser, compiler, action_space, save_files, total_samples,
                           ...
                           allow_mirror=False, allow_mirror_macro=False, max_action_length=128):
