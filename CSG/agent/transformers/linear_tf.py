...
class FastAttenLayer(AttnLayer):
    def __init__(self, nh, hd, dropout, dim_head=16,
                 ff_mult=1, max_seq_len=193, absolute_pos_emb = False):
        ...
        attn = FastAttention(self.hidden_dim, dim_head=dim_head, heads=self.num_heads, 
                             pos_emb=layer_pos_emb, max_seq_len=max_seq_len)
        ff = FeedForward(self.hidden_dim, mult = ff_mult)
        self.self_attn = PreNorm(self.hidden_dim, attn)
        self.ff = PreNorm(self.hidden_dim, ff)


    def forward(self, _src, attn_mask, key_padding_mask=None, return_all=True):
        
        # src = _src.transpose(0, 1)
        attn_mask = (attn_mask==0.0)
        src = self.self_attn(_src, mask=attn_mask, return_all=False)
        _src = src + _src
        _src = self.ff(_src) + _src

        return _src
        
class MultiLayerFastAttnLayer(AttnLayer):

    def __init__(self, nh, hd, dropout, depth=8, max_seq_len=128,
                 dim_head = 64, ff_mult = 4):
        super(AttnLayer, self).__init__()
    ...
    def forward(self, _src, attn_mask, key_padding_mask):
        n, device = _src.shape[1], _src.device
        # x = self.token_emb(x)
        if exists(self.abs_pos_emb):
            pos_emb = self.abs_pos_emb(torch.arange(n, device = device))
            x = x + rearrange(pos_emb, 'n d -> () n d')
