...
from .plad_tf import AttnLayer
from .default_tf import DecoderLayer, LayerNorm, clones

class GatedAttnLayer(AttnLayer):
    def __init__(self, nh, hd, dropout):
        super(GatedAttnLayer, self).__init__(nh, hd, dropout)
        self.gate1 = GatingMechanism(self.hidden_dim)
        self.gate2 = GatingMechanism(self.hidden_dim)
    ...
    def forward(self, _src, attn_mask, key_padding_mask):
        ...
        src = self.gate1(src, src2) 
        src2 = self.l2(self.d2(F.leaky_relu(self.l1(src), .2)))
        src = self.gate2(src, src2)
        return src.transpose(0, 1)


class IdentityReOrderConnection(nn.Module):
    
    def __init__(self, size, dropout):
        super(IdentityReOrderConnection, self).__init__()
        self.norm = LayerNorm(size)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, sublayer, gating):
        "Apply residual connection to any sublayer with the same size."
        return gating(x, self.dropout(sublayer(self.norm(x))))
    
class GatedDecoderLayer(DecoderLayer):
    
    def __init__(self, size, self_attn, src_attn, feed_forward, dropout):
        super(GatedDecoderLayer, self).__init__(size, self_attn, src_attn, feed_forward, dropout)
        self.gate1 = GatingMechanism(self.size)
        self.gate2 = GatingMechanism(self.size)
        self.gate3 = GatingMechanism(self.size)
        self.sublayer = clones(IdentityReOrderConnection(size, dropout), 3)
        
    def forward(self, x, memory, src_mask, tgt_mask):
        "Follow Figure 1 (right) for connections."
        m = memory
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask), self.gate1)
        x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask), self.gate2)
        return self.sublayer[2](x, self.feed_forward, self.gate3)
